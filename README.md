...
5. **Reference** both maps to the same stable point. You can set this point, or leave it automatic.
6. **Compare** the maps:
...
7. **Report:** a self-contained HTML report (light/dark), `metrics.json`, PNG figures, and GeoTIFFs
...
streamlit run app.py            # web app (demo, ASF search, or local files)
pytest                          # tests (readers are checked against a noise-free synthetic truth)
```
...
| `reference_lonlat` | A point you know is stable (bedrock, old building). Auto if empty |
...
- GNSS validation: pull station velocities and plot them on the maps.
