"""Graphical interface (runs in your web browser, on your own computer).
...
def main():
    ...
    with st.sidebar:
        ...
        with st.expander("Validate against published data"):
            st.caption("Independent data to check the InSAR results against.")
            gnss_mode = st.radio("GNSS", ["Off", "NGL stations (automatic download)", "My CSV table"],
                                 index=1 if mode.startswith("Demo") else 0,
                                 help="NGL = Nevada Geodetic Laboratory, free worldwide GNSS velocities. CSV columns: "
                                      "site, lon, lat, up_mm_yr, up_sigma_mm_yr (e.g. a table from a paper).")
            gnss_csv = st.file_uploader("GNSS CSV", type=["csv"]) if gnss_mode == "My CSV table" else None
            pub_file = st.file_uploader("Published velocity map (GeoTIFF / NetCDF)", type=["tif", "tiff", "nc"],
                                        help="e.g. EGMS Ortho (vertical), LiCSAR/COMET or OPERA velocity (LOS), "
                                             "or supplementary data from a paper.")
            if pub_file is not None:
                pub_label = st.text_input("Name", Path(pub_file.name).stem)
                c1, c2 = st.columns(2)
                pub_units = c1.selectbox("Units", ["mm/yr", "cm/yr", "m/yr"])
                pub_comp = c2.selectbox("Component", ["vertical", "LOS"])
                pub_sign = st.radio("Positive values mean", ["up / toward satellite", "down / away"], horizontal=True)
                pub_inc = st.number_input("Incidence angle (° , for LOS maps)", 20.0, 50.0, 39.0) \
                    if pub_comp == "LOS" else None
                pub_period = st.text_input("Period covered (optional)", "")
        ...
        def build_config() -> Config:
            ...
            if gnss_mode == "Off":
                cfg.gnss = None
            elif gnss_mode.startswith("NGL"):
                cfg.gnss = "NGL"
            elif gnss_csv is not None:
                cfg.gnss = save_upload(gnss_csv)
            if pub_file is not None:
                cfg.published_maps = list(cfg.published_maps) + [{
                    "path": save_upload(pub_file), "label": pub_label, "units": pub_units, "component": pub_comp,
                    "sign": 1 if pub_sign.startswith("up") else -1, "incidence_deg": pub_inc, "period": pub_period}]
            return cfg
    ...
    if res:
        ...
        m[3].metric("RMS difference (mm/yr)", f"{c.rmsd_mm_yr:.1f}")
        m[4].metric("Only NISAR can see", f"{c.only_a:.0%}")
        m[5].metric("Moving areas", len(c.hotspots))

        tabs = st.tabs(["Displacement", "Coherence", "Agreement", "Validation", "Pairs", "Interpretation",
                        "Downloads"])
        with tabs[0]:
        ...
        with tabs[3]:
            ...
            if not vnotes:
                st.info("No validation data selected. Open 'Validate against published data' in the sidebar.")
            if "gnss" in res.figures:
                col, _ = st.columns([1, 1])
                col.image(res.figures["gnss"][th], use_container_width=True)
                st.dataframe([{"station": s.site, "lat": round(s.lat, 4), "lon": round(s.lon, 4),
                               "GNSS up mm/yr": round(s.up_mm_yr, 1), "± mm/yr": round(s.up_sigma_mm_yr, 1),
                               "NISAR mm/yr": round(s.insar.get("NISAR", float("nan")), 1),
                               "Sentinel-1 mm/yr": round(s.insar.get("Sentinel-1", float("nan")), 1),
                               "GNSS period": s.period, "note": s.note} for s in res.gnss.stations], hide_index=True)
            for k in range(len(res.published)):
                st.image(res.figures[f"published_{k}"][th], use_container_width=True)
            show_notes(vnotes, st)
        with tabs[4]:
